...
function createHerbCard(herb) {
  ...
  const isFavorite = getFavorites().includes(herb.Herb);
  section.innerHTML = `
    ${herb.Image ? `<img src="${herb.Image}" alt="${herb.Herb}">` : ""}
    <h2>${herb.Herb}
      <button class="favorite-btn ${isFavorite ? 'active' : ''}" onclick="toggleFavorite('${herb.Herb}', this)">★</button>
    </h2>
    <p><strong>Category:</strong> ${herb.Category}</p>
    <p><strong>Tags:</strong> ${formatTags(herb.Tags)}</p>
    <p><strong>Effects:</strong> ${herb.Effects}</p>
    <details><summary><strong>Mechanism of Action</strong></summary><p>${herb["Mechanism of Action"] || 'N/A'}</p></details>
    <details><summary><strong>Pharmacokinetics</strong></summary><p>${herb.Pharmacokinetics || 'N/A'}</p></details>
    <details><summary><strong>Therapeutic Uses</strong></summary><p>${herb["Therapeutic Uses"] || 'N/A'}</p></details>
    <details><summary><strong>Side Effects</strong></summary><p>${herb["Side Effects"] || 'N/A'}</p></details>
    <details><summary><strong>Contraindications</strong></summary><p>${herb.Contraindications || 'N/A'}</p></details>
    <details><summary><strong>Drug Interactions</strong></summary><p>${herb["Drug Interactions"] || 'N/A'}</p></details>
    <details><summary><strong>Toxicity</strong></summary><p>${herb.Toxicity || 'N/A'}</p></details>
  `;
  return section;
}
...
function formatTags(tagString) {
  ...
  return tagString.split(',').map(tag => {
    ...
    return `<button class="tag-filter" onclick="applyTagFilter('${cleanTag}')">${cleanTag}</button>`;
  }).join(' ');
}
...
function renderHerbs() {
  ...
  let filtered = herbData.filter(herb => {
    const haystack = (
      herb.Herb + herb.Effects + herb.Tags + herb.Category
    ).toLowerCase();

    const matchesSearch = haystack.includes(currentFilter.toLowerCase());
    const matchesFavorite = !showFavoritesOnly || getFavorites().includes(herb.Herb);
    return matchesSearch && matchesFavorite;
  });

  if (currentSort === 'alphabetical') {
    filtered.sort((a, b) => a.Herb.localeCompare(b.Herb));
  } else if (currentSort === 'intensity') {
    const order = ['Mild', 'Moderate', 'High'];
    filtered.sort((a, b) => order.indexOf(a.Intensity) - order.indexOf(b.Intensity));
  }
...
function toggleFavorite(herbName, el) {
  let favorites = getFavorites();
  if (favorites.includes(herbName)) {
    favorites = favorites.filter(h => h !== herbName);
...
function exportFavorites() {
  ...
  const csvContent = "data:text/csv;charset=utf-8," + favorites.map(f => `"${f}"`).join("\n");
...
function buildCategoryTabs() {
  const categories = [...new Set(herbData.map(h => h.Category).filter(Boolean))];
...
document.addEventListener('DOMContentLoaded', buildCategoryTabs);
